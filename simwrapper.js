...
IM_EY.REAL_KIADR = 0xf040
IM_EY.REAL_KOADR = 0xf046
...
IM_CY.REAL_KIADR = 0xf040
IM_CY.REAL_KOADR = 0xf046
IM_CY.REAL_KI_FILTER_ADR = 0xf042
IM_CY.REAL_INVERTED_KOADR = 0xf044
...
for (const key in IM_ESP) {
  IM_ESP2[key] = IM_ESP[key];
}
IM_ESP2.HE_CURSOR = IM_ESP2.HE_USER_INPUT_START - 0x40
...
const IM_SOLAR = {}
for (const key in IM_ES) {
	IM_SOLAR[key] = IM_ES[key];
}
IM_SOLAR.ES_STOPTYPEADR = 0xe800
IM_SOLAR.ES_KIADR = 0xe801
IM_SOLAR.ES_KOADR = 0xe802
// IM_SOLAR.VRAMSIZE_WIDTH = 48
// IM_SOLAR.VRAMSIZE_HEIGHT = 16
IM_SOLAR.VRAMSIZE_WIDTH = 64
IM_SOLAR.VRAMSIZE_HEIGHT = 8
IM_SOLAR.ES_DDLEN = 0x48
// IM_SOLAR.ES_DDLEN = 0x100
IM_SOLAR._FULL_SCREEN_WIDTH = 770
IM_SOLAR._FULL_SCREEN_HEIGHT = 198
IM_SOLAR._BASE_SCREEN_WIDTH = 231 // IM_SOLAR._FULL_SCREEN_WIDTH / 10 * 3
IM_SOLAR._BASE_SCREEN_HEIGHT = 59.4 // IM_SOLAR._FULL_SCREEN_HEIGHT / 10 * 3
...
const MODEL_TYPE_CLASSWIZ_GRAPH = "EG";											// ClassWiz models with graphing
...
const IM_MAPPING = {}
IM_MAPPING[MODEL_TYPE_CLASSWIZ_GRAPH] = IM_EY
IM_MAPPING[MODEL_TYPE_CLASSWIZ_NEW] = IM_EY
IM_MAPPING[MODEL_TYPE_CLASSWIZ_OLD] = IM_CY
IM_MAPPING[MODEL_TYPE_ES_PLUS_2] = IM_ESP2
...
class SimWrapper {
	constructor() {
		...
		this._emuROM = true;
		this._realROM = false;
		this._realROMStop = false;
		this._invertedKO = false;
		this._displayColor = [0, 0, 0];
		this._displayColorSetted = false;
		this._messageChannel = new MessageChannel
		this._messageChannel.port1.onmessage = () => this._Execute()
	}

	// Init(rom: string, token: string) : number
	async Init(rom, modelType, romData, keylogData, modelPad, realROM, invertedKO) {
		var result = 1;
		...
		if (rom.endsWith('R')) realROM = true;

		this._model = rom;
		this._modelType = modelType;
		this._emuROM = !realROM;
		this._realROM = realROM;
		this._invertedKO = invertedKO;
		this.IM = IM = IM_MAPPING[modelType];

		switch (this._modelType) {
			case MODEL_TYPE_CLASSWIZ_NEW:
			case MODEL_TYPE_CLASSWIZ_GRAPH:
		...
		switch (this._modelType) {
			...
			case MODEL_TYPE_FC:
				...
				break;
			case MODEL_TYPE_SOLAR:
				this._SetMemoryModel(0);
				this._SetCodeMemorySize(0, 0xffff);
				this._SetDataMemorySize(0xe000, 0xffff);
				this._SetRomWindowSize(0, 0xdfff);
				break;
		}
	...
	RomVersion() {
		...
		if (_api) {
			...
			var sumStr = (sum[1] * 0x100 + sum[0]).toString(16).toUpperCase().padStart(4, '0');
	...
	GetDisplayImageData(oldImageData, shadow = false) {
		if (!oldImageData || oldImageData.width !== IM.VRAMSIZE_WIDTH || oldImageData.height !== IM.VRAMSIZE_HEIGHT || !this._displayColorSetted) {
			oldImageData = new ImageData(IM.VRAMSIZE_WIDTH, IM.VRAMSIZE_HEIGHT)
			const color = this._displayColor;
			for (let i = 0; i < oldImageData.data.length; i += 4) {
				oldImageData.data[i] = color[0];
				oldImageData.data[i + 1] = color[1];
				oldImageData.data[i + 2] = color[2];
				oldImageData.data[i + 3] = 255;
			}
			this._displayColorSetted = true;
		}
				...
				case MODEL_TYPE_SOLAR:
					for (let row = 0; row < IM.VRAMSIZE_HEIGHT; ++row) {
						for (let col = 0; col < IM.VRAMSIZE_WIDTH / 8; ++col) {
							let colValue = this._vram[row*IM.VRAMSIZE_HEIGHT+col];
							let colX = col * 8;
	...
	GetDisplayBraille() {
		...
		const height = IM.VRAMSIZE_HEIGHT;
		const width = IM.VRAMSIZE_WIDTH;
		const heightFixed = height % 4 === 0 ? height : height + 4 - (height % 4);
		const widthFixed = width % 2 === 0 ? width : width + 2 - (width % 2);
		const vram = new Array(heightFixed * widthFixed);

		const fillVRAM = (getBit) => {
			for (let row = 0; row < height; ++row) {
				const rowOffset = IM.VRAM_DOTSTART_OFFSET + row * IM.VRAMSIZE_LINE;
				for (let col = 0; col < width; ++col) {
					const index = row * width + col;
					vram[index] = getBit(rowOffset, col);
				}
			}
		};
		...
		switch (this._modelType) {
			case MODEL_TYPE_CLASSWIZ_NEW:
			case MODEL_TYPE_CLASSWIZ_GRAPH:
				fillVRAM((rowOffset, col) => !!this._vram[rowOffset + col]);
				break;
			case MODEL_TYPE_CLASSWIZ_OLD:
			case MODEL_TYPE_ES_PLUS:
			case MODEL_TYPE_ES_PLUS_2:
			case MODEL_TYPE_ES:
			case MODEL_TYPE_FC:
				fillVRAM((rowOffset, col) => {
					const byte = this._vram[rowOffset + Math.floor(col / 8)];
					return !!(byte & (1 << (7 - (col % 8))));
				});
				break;
			case MODEL_TYPE_SOLAR:
				// TODO
		...
		for (let row = 0; row < heightFixed; row += 4) {
			for (let col = 0; col < widthFixed; col += 2) {
				const rowOffset = row * widthFixed + col;
				const a8 = !!vram[rowOffset];
				const a5 = !!vram[rowOffset + 1];
				const a7 = !!vram[rowOffset + widthFixed];
				const a4 = !!vram[rowOffset + widthFixed + 1];
				const a6 = !!vram[rowOffset + 2 * widthFixed];
				const a3 = !!vram[rowOffset + 2 * widthFixed + 1];
				const a2 = !!vram[rowOffset + 3 * widthFixed];
				const a1 = !!vram[rowOffset + 3 * widthFixed + 1];

				const code = (a1 << 7) | (a2 << 6) | (a3 << 5) | (a4 << 4) | (a5 << 3) | (a6 << 2) | (a7 << 1) | a8;
				const brailleChar = String.fromCodePoint(0x2800 + code);
				text += brailleChar;
	...
	GetSymbolState(data) {
      ...
      case MODEL_TYPE_CLASSWIZ_GRAPH:
        ...
        break;
			...
			case MODEL_TYPE_SOLAR:
				status_bar = [
					data[0x11] & (1 << 6),  // SHIFT 11.6
					data[0x11] & (1 << 2),  // MODE 11.2
					data[0x12] & (1 << 6),  // STO 12.6
					data[0x12] & (1 << 2),  // RCL 12.2
					data[0x13] & (1 << 6),  // hyp 13.6
					data[0x13] & (1 << 2),  // M 13.2
					data[0x14] & (1 << 6),  // K 14.6
					data[0x14] & (1 << 2),  // DEG 14.2
					data[0x15] & (1 << 6),  // RAD 15.6
					data[0x15] & (1 << 2),  // GRA 15.2
					data[0x16] & (1 << 4),  // FIX 16.4
					data[0x16] & (1 << 2),  // SCI 16.2
					data[0x16] & (1 << 0),  // SD 16.0
					data[0x08] & (1 << 1),  // 08.1
					data[0x01] & (1 << 5),  // 01.5
					data[0x01] & (1 << 6),  // 01.6
					data[0x01] & (1 << 4),  // 01.4
	...
	async SimulateDiag(on) {
		if (on) {
			switch (this._modelType) {
				...
				case MODEL_TYPE_CLASSWIZ_GRAPH:
				case MODEL_TYPE_SOLAR:
					this._WriteDataMemory(IM.REAL_KIADR, 1, new Uint8Array([0xdb]))
					break;
				case MODEL_TYPE_CLASSWIZ_OLD:
				case MODEL_TYPE_ES_PLUS:
				case MODEL_TYPE_ES_PLUS_2:
				case MODEL_TYPE_ES:
				case MODEL_TYPE_FC:
					this._WriteDataMemory(IM.REAL_KIADR, 1, new Uint8Array([0x7b]))
					break
			}
		} else {
			this._WriteDataMemory(IM.REAL_KIADR, 1, new Uint8Array([0x0]))
		}
	}

	async SimulateFatDiag(on) {
		if (on) {
			this._WriteDataMemory(IM.REAL_KIADR, 1, new Uint8Array([0xe7]))
		} else {
			this._WriteDataMemory(IM.REAL_KIADR, 1, new Uint8Array([0x0]))
		}
	}

	async SetDisplayColor(color) {
		this._displayColor = color;
		this._displayColorSetted = false;
	}

	async GetDisplayColor() {
		return this._displayColor;
	}

  async GetREGDefinition() {
    return REG;
  }
	...
	async _Execute() {
		if (_api && this._simRunning) {
			...
			// “Rinse and repeat”.
			// We need to queue a macrotask without delay, so as to continue execution as soon as possible if nothing special happens.
			// setTimeout imposes a minimum of 4ms delay in recursive calls, which is why the ClassPad simulator calculates so slowly.
			// setImmediate is designed for this exact scenario, but it is only available in Node.js.
			// We have to hack around with MessageChannel in a worker.
			// https://github.com/YuzuJS/setImmediate
			this._messageChannel.port2.postMessage(undefined)
		}
	}
	...
	_WaitProcess() {
		...
		if (this._realROM) {
			this._WriteDataMemory(IM.REAL_KOADR, 1, kiko);
			kiko[0] = 0xff;
			this._WriteDataMemory(IM.REAL_KIADR, 1, kiko);
		}
		this._WriteBitDataMemory(REG.IRQ0, IM.STOP_RELEASE_BIT, 1);	// �?„‚èž‚Ý�?—�?(Timer STOP‚©‚ç‚Ì•œ‹A)
		this._realROMStop = false;
	}
	...
	async _KeyProcess() {
		...
		while(CurCounter < 36) {
			...
			if (this._emuROM) await _sleep(15); else await _sleep(1);
			CurCounter++;
		}
		...
		if (this._emuROM) {
			...
			kiko[0] = ko;
			this._WriteDataMemory(IM.ES_KOADR, 1, kiko);							// ko‚ð‘‚«ž‚�?
			this._WriteBitDataMemory(REG.IRQ0, bit, 1);								// �?„‚èž‚Ý�?—�?(Timer STOP‚©‚ç‚Ì•œ‹A)
		}
		if (this._realROM) {
			this._realROMStop = true;
			this._WriteBitDataMemory(REG.IRQ0, bit, 1);
			kiko[0] = ko;
			this._WriteDataMemory(IM.REAL_KOADR, 1, kiko);
			let realKi = 0xff;
			realKi &= ~ki;
